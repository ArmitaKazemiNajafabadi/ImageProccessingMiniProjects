...
def do_the_thing(pos_i,pos_j,r,source,target):
    ...
    source_1_gus = cv2.GaussianBlur(source_1,(21,21),6)
    target_1_gus = cv2.GaussianBlur(target_1,(21,21),6)
    ...
    mask_blur_1 =  cv2.GaussianBlur(mask_1,(35,35),20) * mask_1 
...
source = cv2.imread(os.getcwd()+'\\'+'3source.jpg',cv2.IMREAD_UNCHANGED)
target = cv2.imread(os.getcwd()+'\\'+'3target.jpg',cv2.IMREAD_UNCHANGED)

# # ,305,1005
# pos_i = 305
# pos_j = 1005
source = cv2.resize(source,(source.shape[1]//5,source.shape[0]//5))
...
mask[pos_i:pos_i+h,pos_j:pos_j+w,:] = np.greater(source,25)
...
cv2.imwrite('3res.jpg',res.astype('uint8'))
