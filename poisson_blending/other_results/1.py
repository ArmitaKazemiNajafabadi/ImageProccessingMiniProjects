...
source = cv2.imread(os.getcwd()+'\\'+'1source.jpg',cv2.IMREAD_UNCHANGED)
target = cv2.imread(os.getcwd()+'\\'+'1target.jpg',cv2.IMREAD_UNCHANGED)
res = target[:,:,:]
...
source = cv2.resize(source,(source.shape[1]//4,source.shape[0]//4))
...
for r in range(0,3):
    ...
    part_res  = part_res*(1-helper) + up*helper
